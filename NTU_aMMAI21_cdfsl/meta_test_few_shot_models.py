...
def meta_test(novel_loader, n_query = 15, task='fsl', finetune=True, n_pseudo=100, n_way = 5, n_support = 5):
    ...
    for ti, (x, y) in enumerate(novel_loader):
        ...
        if params.method == 'protonet':
            ...
            pretrained_model = SubspaceNet(model_dict[params.model], n_way=n_way, n_support=n_support)
        elif params.method == 'subspace_plus':
            pretrained_model = SubspaceNetPlus(model_dict[params.model], n_way=n_way, n_support=n_support)
        elif params.method == 'subspace_strong':
            pretrained_model = SubspaceNetStrong(model_dict[params.model], n_way=n_way, n_support=n_support)
        ...
        with torch.no_grad():
            if params.method == 'subspace':
                scores, _ = pretrained_model.set_forward(x_var.cuda())
            elif params.method == 'subspace_plus' or params.method == 'subspace_strong':
                scores, _, _ = pretrained_model.set_forward(x_var.cuda())
            else:
...
if __name__=='__main__':
    ...
    ##################################################################
    novel_loaders = []
    def get_novel_loader(dataset_name):
    ...
    for idx, dataset_name in enumerate(dataset_names):
        ...
        novel_loader = get_novel_loader(dataset_name)
